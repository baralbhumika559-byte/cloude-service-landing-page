...
// ---------------------------------------------------------------------------
// Flodesk embed for "Book a consultation call" (form id 6a91b97715d1aba2900c08a4)
//
// This is the exact embed Flodesk generated — markup, CSS, field names,
// hidden inputs, the submit action, and the two loader scripts are all
// untouched. The ONLY intentional change is inside the base64 `data-ff-config`
// block below: `onSuccess.mode` was flipped from "redirect" to "message".
//
// Why: the original config redirects the browser (immediately, with no
// delay) to an external Flodesk-hosted thank-you URL the moment Flodesk's
// own script sees a successful submit. That would hijack navigation away
// from this Next.js site before we ever get a chance to send the visitor to
// our own /thank-you page, and with no delay for Flodesk's post-submit
// automation to settle. Switching to "message" mode makes Flodesk show its
// own inline success state (it sets data-ff-stage="success" on the root
// element) instead of navigating away. We watch for that attribute change
// below and, only after it fires (i.e. only after Flodesk has confirmed the
// submission), wait a couple of seconds and then route to /thank-you
// ourselves. Lead capture, field names, and the automation trigger (which
// fires server-side on Flodesk's end the moment the POST succeeds) are
// completely unaffected by this — it only changes what the *browser* does
// visually after a successful submit.
// ---------------------------------------------------------------------------

const FORM_ID = "6a91b97715d1aba2900c08a4";
const ROOT_CLASS = "ff-6a91b97715d1aba2900c08a4";
...
// from "redirect" to "message":
// {
//   "trigger": { "mode": "immediately", "value": 0 },
//   "onSuccess": {
//     "mode": "message",
//     "message": "<div data-paragraph=\"true\">Thank you for subscribing!</div>",
//     "redirectUrl": "https://consultation.bhumikabaral.com.np/thank-you"
//   },
//   "coi": false,
...
// }
const FF_CONFIG =
  "";
...
// Our own additions on top of Flodesk's CSS (same selector specificity,
// rendered after it in the DOM, so these win on source order without
// needing !important or touching a single Flodesk rule above).
const BRAND_OVERRIDES_CSS = `
  ...
  [data-ff-el="root"].${ROOT_CLASS} .${ROOT_CLASS}__button {
    ...
    font-family: var(--font-body), system-ui, sans-serif;
    letter-spacing: 0;
    box-shadow: 0 1px 2px rgba(16,24,40,0.04), 0 12px 32px -8px rgba(41,84,224,0.16);
...
// Verbatim Flodesk markup — only the data-ff-config value differs from what
// Flodesk generated (onSuccess.mode "redirect" -> "message"; see note above).
// Field names, hidden inputs, the submit action, and data-ff-* attributes
// are all exactly as provided.
const FORM_MARKUP = `
  ...
  <div class="${ROOT_CLASS}__container">
    <div class="${ROOT_CLASS}__wrapper">
      <form class="${ROOT_CLASS}__form" action="https://form.flodesk.com/forms/${FORM_ID}/submit" method="post" data-ff-el="form">
        <div class="${ROOT_CLASS}__title">
          <div style="min-height: 1em"><div></div></div>
        </div>
        <div class="${ROOT_CLASS}__subtitle">
          <div style="min-height: 1.6em"><div></div></div>
        </div>
        <div class="${ROOT_CLASS}__content fd-form-content" data-ff-el="content">
          <div class="${ROOT_CLASS}__fields" data-ff-el="fields">
            <div class="${ROOT_CLASS}__field fd-form-group">
              <input id="ff-${FORM_ID}-firstName" class="${ROOT_CLASS}__control fd-form-control" type="text" maxlength="255" name="firstName" placeholder="First name" data-ff-tab="firstName::email" required />
              <label for="ff-${FORM_ID}-firstName" class="${ROOT_CLASS}__label fd-form-label"><div><div>First name</div></div></label>
            </div>
            <div class="${ROOT_CLASS}__field fd-form-group">
              <input id="ff-${FORM_ID}-email" class="${ROOT_CLASS}__control fd-form-control" type="text" maxlength="255" name="email" placeholder="Email address" data-ff-tab="email:firstName:fields.whatsappNumber" required />
              <label for="ff-${FORM_ID}-email" class="${ROOT_CLASS}__label fd-form-label"><div><div>Email address</div></div></label>
            </div>
            <div class="${ROOT_CLASS}__field fd-form-group">
              <input id="ff-${FORM_ID}-ZZEFQWkMx3" class="${ROOT_CLASS}__control fd-form-control" type="text" maxlength="255" name="fields.whatsappNumber" placeholder="WhatsApp Number" data-ff-tab="fields.whatsappNumber:email:fields.businessName" required />
              <label for="ff-${FORM_ID}-ZZEFQWkMx3" class="${ROOT_CLASS}__label fd-form-label"><div><div>WhatsApp Number</div></div></label>
            </div>
            <div class="${ROOT_CLASS}__field fd-form-group">
              <input id="ff-${FORM_ID}-cQEkQ66hLq" class="${ROOT_CLASS}__control fd-form-control" type="text" maxlength="255" name="fields.businessName" placeholder="Business Name" data-ff-tab="fields.businessName:fields.whatsappNumber:fields.websiteLink" required />
              <label for="ff-${FORM_ID}-cQEkQ66hLq" class="${ROOT_CLASS}__label fd-form-label"><div><div>Business Name</div></div></label>
            </div>
            <div class="${ROOT_CLASS}__field fd-form-group">
              <input id="ff-${FORM_ID}-SmHF6LfU6G" class="${ROOT_CLASS}__control fd-form-control" type="text" maxlength="255" name="fields.websiteLink" placeholder="Website/Facebook Link" data-ff-tab="fields.websiteLink:fields.businessName:submit" required />
              <label for="ff-${FORM_ID}-SmHF6LfU6G" class="${ROOT_CLASS}__label fd-form-label"><div><div>Website/Facebook Link</div></div></label>
            </div>
          ...
          <div class="${ROOT_CLASS}__footer" data-ff-el="footer">
            <button type="submit" class="${ROOT_CLASS}__button fd-btn" data-ff-el="submit" data-ff-tab="submit">
              <div><span data-draw-element="editable">Book a consultation call</span></div>
            </button>
          </div>
        </div>
        <div class="${ROOT_CLASS}__success fd-form-success" data-ff-el="success">
          <div class="${ROOT_CLASS}__success-message">
            <div><div><div data-paragraph="true">You&rsquo;ve successfully signed up! Check your email for details.</div></div></div>
          </div>
        </div>
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => {
      ...
      if (root.getAttribute("data-ff-stage") === "success") {
        redirected = true;
        // Let Flodesk's own success UI show, and give its post-submit
        // automation a moment to finish, before we navigate away.
        window.setTimeout(() => {
